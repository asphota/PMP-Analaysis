...
fitting_dist1 <- function (x){
 ...
 # fit.GP <- fevd(x, type = "GP", threshold = min(x))
  ...
  P1000_gumbel <- ci(fit.gumbel, return.period = 10000, alpha = .05)[2]
  P1000_GEV <- ci(fit.GEV, return.period = 10000, alpha = .05)[2]
 # P100_GP <- ci(fit.GP, alpha = .5)[2]
  
  CI_P1000_gumbel1 <- ci(fit.gumbel, return.period = 10000, alpha = .05)[1]
  CI_P1000_gumbel2 <- ci(fit.gumbel, return.period = 10000, alpha = .05)[3]
  
  CI_P1000_GEV1 <- ci(fit.GEV, return.period = 10000, alpha = .05)[1]
  CI_P1000_GEV2 <- ci(fit.GEV, return.period = 10000, alpha = .05)[3]
  
  
  return_1000_year <- c(Length_record,Max_value,  CI_P1000_gumbel1, P1000_gumbel, CI_P1000_gumbel2, CI_P1000_GEV1, P1000_GEV, CI_P1000_GEV2)#, P100_GP)
  names(return_1000_year) <- c("Length","Maximum",  "CI_Gumbel1","Gumbel", "CI_Gumbel2", "CI_GEV1","GEV","CI_GEV2")#, "GP")
  return(return_1000_year)
}
...
fitting_dist2 <- function (x,thres){
  ...
  # Confidence interval and return levels
  P1000_GP <- ci(fit.GP, alpha = .05)[2]
  
  CI_P1000_GP1 <- ci(fit.GP, alpha = .05)[1]
  CI_P1000_GP2 <- ci(fit.GP, alpha = .05)[3]

  return_1000_year <- c(CI_P1000_GP1, P1000_GP, CI_P1000_GP2)#, P100_GP)
  names(return_1000_year) <- c("CI_GP1","GP", "CI_GP2")#, "GP")
  return(return_1000_year)
}
...
x <- file_x_max_nz
ci(fevd(x, type = "Gumbel"), alpha = 0.05, return.period = 1000)
...
write.csv(Complete_list, "E:\\My_papers\\Drafts\\PMP_Upper_Arun\\GIS\\1000_year_precip.csv")
